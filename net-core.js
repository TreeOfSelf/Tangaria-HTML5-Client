...
class MAngbandNetworkClient {
	...
	net_read() {
		...
		while (len > 0) {
			...
			//let list = [7,40,19,126,38,24,49,54,23,27]
			//let list = [PKT_END,PKT_KEEPALIVE,PKT_SKILLS,PKT_WEIGHT]; 
			//if(list.indexOf(pkt)==-1) console.log(pkt,handler);
			console.log(pkt,handler);
			if (!handler) {
				//Util.Error("UNDEFINED PACKET " + pkt + " (previous packet:" + this.last_pkt + ")");
				//this.protocol.close();
				//this.net.close();
				//return;
				console.log("UNDEFINED PACKET " + pkt + " (previous packet:" + this.last_pkt + ")");
				return
			}
			try {
